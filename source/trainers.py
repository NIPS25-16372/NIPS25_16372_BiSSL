...
def train_object_detector(
    ...
    device: torch.device,
    lr_scheduler: Optional[CosineLRSchedulerWithWarmup | ConstantLR] = None,
    return_loss_avg_total: bool = False,
) -> Optional[float]:
    loss_avg_total = 0
    model.train()
    ...
    for imgs, targets in dataloader:
        imgs, targets = (
            [img.to(device, non_blocking=True) for img in imgs],
            [{k: v.to(device) for k, v in t.items()} for t in targets],
        )

        # with torch.cuda.amp.autocast(): # type : ignore
        loss_dict = model(imgs, targets=targets)
        loss = sum(loss for loss in loss_dict.values())
        optimizer.zero_grad()
        loss.backward()  # type: ignore
        optimizer.step()

        loss_avg_total += loss.item()  # type: ignore

        if lr_scheduler is not None:
            lr_scheduler.step()

    print(
        f"Avg loss over batch no. 1-{len(dataloader)} / {len(dataloader)}: {loss_avg_total/len(dataloader):>5f}",
    )

    if return_loss_avg_total:
        return loss_avg_total / len(dataloader)
